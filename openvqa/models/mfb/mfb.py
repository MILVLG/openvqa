...
# -------------------------------------------------------------
# ---- Multi-Model Factor Bilinear Pooling Co-Attention----
# -------------------------------------------------------------

# MFB Module (Image Attention)
...
# MFB Module (Outer)
class MFB2(nn.Module):
    def __init__(self, __C):
        super(MFB2, self).__init__()
        self.__C = __C
        self.JOINT_EMB_SIZE = __C.MFB_FACTOR_NUM * __C.MFB_OUT_DIM      # 5*1000=5000
        self.FC_q = nn.Linear(__C.HIDDEN_SIZE*__C.NUM_QUESTION_GLIMPSE, self.JOINT_EMB_SIZE)    # 1024*2 -> 5000
    ...
    def forward(self, que_feat, img_feat):
        ...
        z = que_feat * img_feat             # N x 5000
        z = self.dropout(z)                 # N x 5000
...
class QuestionAttention(nn.Module):
    def __init__(self, __C):
        ...
        self.dropout_lstm = nn.Dropout(__C.LSTM_DROPOUT_RATIO)
